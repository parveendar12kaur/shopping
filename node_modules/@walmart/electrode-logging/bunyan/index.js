/* copied from https://github.com/trentm/node-bunyan/commit/fe31b83e42d9c7f784e83fdcc528a7c76e0dacae */
...
function Logger(options, _childOptions, _childSimple) {
    ...
    if (parent) {
        ...
        for (var i = 0; i < parent.streams.length; i++) {
            var s = objDup(parent.streams[i]);
            s.closeOnExit = false; // Don't own parent stream.
            this.streams.push(s);
        }
        this.serializers = objDup(parent.serializers);
        this.src = parent.src;
        this.fields = objDup(parent.fields);
        if (options.level) {
    ...
    delete fields.src;
    if (this.serializers) {
        this._applySerializers(fields, fields);
    }
 ...
 */
Logger.prototype._applySerializers = function (fields, target, excludeFields) {
  xxx('_applySerializers: excludeFields', excludeFields);

  var field;
  // Check each serializer against these (presuming number of serializers
  // is typically less than number of fields).
  for( var name in this.serializers ) {
    field = fields[name];
    if (field === undefined ||
      (excludeFields && excludeFields[name])) continue;

    xxx('_applySerializers; apply to "%s" key', name)
    try {
      target[name] = this.serializers[name](field, fields, this);
    } catch (err) {
      _warn(format('bunyan: ERROR: Exception thrown from the "%s" '
          + 'Bunyan serializer. This should never happen. This is a bug '
          + 'in that serializer function.\n%s',
          name, err.stack || err));
      target[name] = format('(Error in Bunyan log "%s" serializer '
...
function mkRecord(log, minLevel, args) {
    ...
    if (fields) {
      objCopy(rec, fields);
      if (log.serializers) {
        log._applySerializers(fields, rec, excludeFields);
      }
    }
