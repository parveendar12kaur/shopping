...
```
### Using a proxy for all API Calls including your swagger.json url

Add a `requestInterceptor` to your configuration to set a proxy.  :
...
```
*Note* Having both 'proxy' and 'requestInterceptor' will not cause any issues. In that case, 'requestInterceptor' will take precedence.

### Using a proxy for just call API calls  
